...
class MultiHeadAttention:
    ...
    def backward(self, d_output):
        ...
        # Split heads to match attention layout (N, H, L, d_k)
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        """
        # Attention scores are (N, H, L, S). Key padding (N, S) masks keys for all query positions.
        key_mask = None
        ...
        if key_mask is not None and attention_mask is not None:
            return np.logical_or(key_mask, attention_mask)
    ...
    def _split_heads(self, x):
        ...
        """
        # (N, L, E) -> (N, L, H, d_k) -> (N, H, L, d_k)
        d_k = self.E // self.num_heads
        x = x.reshape(self.N, self.L, self.num_heads, d_k)
        x = np.swapaxes(x, 1, 2)
        return x
