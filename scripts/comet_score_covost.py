#!/usr/bin/env python3
"""Score COMET for all covost_cascaded/* experiment directories."""
...
def main():
    ...
    src_path = REPO / "data/covost_enzh/subset100_source_asr.txt"
    # Reference without character spaces for COMET
    tgt_raw = REPO / "data/covost_enzh/subset100_target_zh.txt"
    srcs = [l.strip() for l in open(src_path)]
    refs = [dejoin_zh(l) for l in open(tgt_raw)]
    print(f"Loaded {len(srcs)} src / {len(refs)} ref")

    exps = sorted((REPO / "outputs/covost_cascaded").glob("*"))
    exps = [e for e in exps if (e / "instances.log").exists() and (e / "scores").exists()]
    print(f"Found {len(exps)} experiments")

    # Lazy-load COMET once
    model = None
    results = []
    for exp in exps:
        ...
        # Extract predictions (sorted by index, dejoined)
        recs = []
