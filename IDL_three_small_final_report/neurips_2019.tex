...
text agent); an end-to-end streaming-ASR stack is left as future work. The three stages are:
...
\textbf{+0.12} source-word extra lag. On the WMT19 EN$\rightarrow$ZH text benchmark the full
configuration (TC with a nucleus top-$p$=0.95 consensus pool) reaches
\textbf{35.12 BLEU at AL 9.64}, yielding \textbf{+6.97 BLEU (28.15$\rightarrow$35.12)} and
\textbf{+0.078 COMET} over the wait-$k$=5 baseline; the fixed top-$k$=10 variant already gives
$+6.01$ BLEU, and the same token-level signal transfers to a 600M-parameter NLLB baseline.
Ablations over the number of sampled futures $K\!\in\!\{4,6,10,15\}$, the per-future top-$k$
pool size, and the consensus pool mode (top-$k$, nucleus top-$p$, min-$p$) confirm the gain is
robust and identify nucleus top-$p{=}0.95$ as the best pool rule ($+0.96$ BLEU on WMT19 full
over fixed top-$k$).
\end{abstract}
...
$K$=10). ``Pool size'' is the median number of candidates per future after filtering.
Bold marks the best rule in each family. The nucleus top-$p{=}0.95$ setting is the one we run
on the full test set in Table~\ref{tab:wmt}.}
\label{tab:poolmode}
\centering
\small
\begin{tabular}{llrrr}
